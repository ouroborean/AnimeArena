...
func split_desc():
	return [
		"Target ally wields Patty for 4 turns, and The Thompson Sisters gain 10 Damage Reduction",
		...
		["Cannot target a character already wielding Liz, unless that character is Death the Kid", Color.ORANGE_RED],
		["While active, this skill is replaced by Wave Compression: Patty", Color.AQUA]
	]
...
func execute(user, battle):
	...
	for target in user.targeter.targets:
		var color_change = Effect.color_change_effect(4, 1, 9)
		color_change.set_source(self)
		Character.add_allied_effect(context, user, target, color_change)
		var trigger = Effect.trigger_effect(Trigger.always(harmful_use_trigger), EffectType.Type.HARMFUL_USE_TRIGGER, 9, "If this character uses a Harmful skill, Patty will deal 10 damage to their primary target")
		trigger.set_source(self)
		Character.add_allied_effect(context, user, target, trigger)
		if target.path_name == "kid" and user.moveset.base_abilities[0].cooldown_remaining == 0:
			kid_target = true
			var color_change2 = Effect.color_change_effect(4, 0, 9)
			color_change2.set_source(user.moveset.base_abilities[0])
			Character.add_allied_effect(context, user, target, color_change2)
			var trigger2 = Effect.trigger_effect(Trigger.always(harmful_use_trigger), EffectType.Type.HARMFUL_USE_TRIGGER, 9, "If this character uses a Harmful skill, Liz will deal 10 damage to their primary target")
			trigger2.set_source(user.moveset.base_abilities[0])
	...
	if kid_target and user.moveset.base_abilities[0].cooldown_remaining == 0:
		user.moveset.base_abilities[0].cooldown_remaining = 2
		var dr2 = Effect.damage_reduction_effect(10, 8)
		dr2.set_source(user.moveset.base_abilities[0])
		Character.add_allied_effect(context, user, user, dr2)
		var swap2 = Effect.ability_swap_effect(5, 0, user, 9)
		swap2.set_source(user.moveset.base_abilities[0])
...
# allowed to receive Patty on top of Liz.
# The wield handle is the HARMFUL_USE_TRIGGER the transform plants, the same one
# lizandpatty3:38-39 and lizandpatty5 read. There is no wielder MARK to key on.
func _wield_conflict(user, character) -> bool:
	if character.path_name == "kid":
		return false
	return character.has_effect("Transform: Liz", EffectType.Type.HARMFUL_USE_TRIGGER, user) != null


func extra_usable(user):
	# Mirrors target(). Without it the button stays lit on a turn where every ally is
	# already holding Liz, and the skill only fails once it has been paid for.
