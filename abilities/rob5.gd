...
func describe(user):
	...
	return "Deals 35 Piercing damage to target enemy. If the enemy is affected by Shield, this skill deals Affliction damage instead. Swaps with Rokushiki: Shigan on use. Bypasses Invulnerability."

func split_desc():
	return [
		"Deals 35 Piercing damage to target enemy, Bypassing Invulnerability",
		["Deals Affliction damage instead if the target has Shield", Color.CADET_BLUE],
		["Swaps with Rokushiki: Shigan on use", Color.AQUA]
	]

func execute(user, battle):
	var context = QueryContext.from_game_state(user, battle)
	for target in user.targeter.targets:
		var damage_type = DamageType.Type.PIERCING
		var shields = target.get_effects_by_type(EffectType.Type.SHIELD)
		if shields:
			user.manually_advance_mission(10, 1)
			damage_type = DamageType.Type.AFFLICTION
		Character.resolve_damage(context, target, 35, damage_type)
	
	
	user.effects.remove_effect("Rokushiki: Shigan", EffectType.Type.ABILITY_SWAP)
	activate_neko_neko()
