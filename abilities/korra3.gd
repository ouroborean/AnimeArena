...
# Earth Control. Counters the first Physical skill an enemy uses, and swaps ALL of Korra's slots to
# their elemental attacks for the follow-up turn. If the counter lands, Earth Area Attack costs 1 Random
# less for 1 turn.

func describe(user):
	return "For 1 turn, counters the first Physical skill used by one enemy. Swaps all of Korra's skills to their elemental attacks. If the counter succeeds, Earth Area Attack costs 1 Random less for 1 turn. Invisible."

func split_desc():
	return [
		"For 1 turn, counters the first Physical skill used by target enemy",
		["Swaps all of Korra's skills to their elemental attacks", Color.AQUAMARINE],
		["If the counter succeeds, Earth Area Attack costs 1 Random less for 1 turn", Color.CADET_BLUE],
...
func execute(user, battle):
	...
	for target in user.targeter.targets:
		var counter = Effect.counter_effect(Trigger.always(earth_control_countered), EffectType.Type.COUNTER_USE, 2, "The first Physical skill used by this character will be countered.", ["Physical"])
...
func earth_control_countered(context):
	var korra = context['effect'].user
	if korra != null and not (korra.dead or korra.banished):
		var discount = Effect.cost_mod_effect(-1, 2, Energy.Type.RANDOM, ["Earth Area Attack"])
