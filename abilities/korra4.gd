...
# Water Control. Counters the first Energy skill an enemy uses, and swaps ALL of Korra's slots to their
# elemental attacks for the follow-up turn. If the counter lands, Water Arm Stun costs 1 Random less for
# 1 turn.

func describe(user):
	return "For 1 turn, counters the first Energy skill used by one enemy. Swaps all of Korra's skills to their elemental attacks. If the counter succeeds, Water Arm Stun costs 1 Random less for 1 turn. Invisible."

func split_desc():
	return [
		"For 1 turn, counters the first Energy skill used by target enemy",
		["Swaps all of Korra's skills to their elemental attacks", Color.AQUAMARINE],
		["If the counter succeeds, Water Arm Stun costs 1 Random less for 1 turn", Color.CADET_BLUE],
...
func execute(user, battle):
	...
	for target in user.targeter.targets:
		var counter = Effect.counter_effect(Trigger.always(water_control_countered), EffectType.Type.COUNTER_USE, 2, "The first Energy skill used by this character will be countered.", ["Energy"])
...
func water_control_countered(context):
	var korra = context['effect'].user
	if korra != null and not (korra.dead or korra.banished):
		var discount = Effect.cost_mod_effect(-1, 2, Energy.Type.RANDOM, ["Water Arm Stun"])
